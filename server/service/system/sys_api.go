...
func (apiService *ApiService) CreateApi(api system.SysApi) (err error) {
	if !errors.Is(global.OPM_DB.Where("path = ? AND method = ?", api.Path, api.Method).First(&system.SysApi{}).Error, gorm.ErrRecordNotFound) {
		return errors.New("存在相同api")
	}
	return global.OPM_DB.Create(&api).Error
}
...
func (apiService *ApiService) DeleteApi(api system.SysApi) (err error) {
	...
	err = global.OPM_DB.Where("id = ?", api.ID).First(&entity).Error // 根据id查询api记录
	if errors.Is(err, gorm.ErrRecordNotFound) {                      // api记录不存在
		return err
	}
	err = global.OPM_DB.Delete(&entity).Error
	if err != nil {
		return err
	}
...
func (apiService *ApiService) GetAPIInfoList(api system.SysApi, info request.PageInfo, order string, desc bool) (list interface{}, total int64, err error) {
	...
	db := global.OPM_DB.Model(&system.SysApi{})
...
func (apiService *ApiService) GetAllApis() (apis []system.SysApi, err error) {
	err = global.OPM_DB.Find(&apis).Error
	return
}
...
func (apiService *ApiService) GetApiById(id int) (api system.SysApi, err error) {
	err = global.OPM_DB.Where("id = ?", id).First(&api).Error
	return
}
...
func (apiService *ApiService) UpdateApi(api system.SysApi) (err error) {
	...
	err = global.OPM_DB.Where("id = ?", api.ID).First(&oldA).Error
	...
	if err != nil {
		...
		if err != nil {
			...
			err = global.OPM_DB.Save(&api).Error
		}
	}
	return err
}
...
func (apiService *ApiService) DeleteApisByIds(ids request.IdsReq) (err error) {
	...
	err = global.OPM_DB.Find(&apis, "id in ?", ids.Ids).Delete(&apis).Error
	if err != nil {
		return err
	} else {
