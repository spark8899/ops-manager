...
import (
	...
	email_response "github.com/spark8899/ops-manager/server/plugin/email/model/response"
	"github.com/spark8899/ops-manager/server/plugin/email/service"
	"go.uber.org/zap"
)
...
func (s *EmailApi) EmailTest(c *gin.Context) {
	...
	if err != nil {
		global.OPM_LOG.Error("发送失败!", zap.Error(err))
