...
{{ if .NeedModel }}	"github.com/spark8899/ops-manager/server/plugin/{{ .Snake}}/model" {{ end }}
	"github.com/spark8899/ops-manager/server/plugin/{{ .Snake}}/service"
...
func (p *{{ .PlugName}}Api) ApiName(c *gin.Context) {
    ...
    {{ end }}
		...
		global.OPM_LOG.Error("失败!", zap.Error(err))
