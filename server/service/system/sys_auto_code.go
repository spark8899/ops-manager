...
import (
	...
	ast2 "github.com/spark8899/ops-manager/server/utils/ast"

	cp "github.com/otiai10/copy"
	"github.com/spark8899/ops-manager/server/resource/autocode_template/subcontract"
	"go.uber.org/zap"
...
func Init(Package string) {
	injectionPaths = []injectionMeta{
		{
			path: filepath.Join(global.OPM_CONFIG.AutoCode.Root,
				global.OPM_CONFIG.AutoCode.Server, fmt.Sprintf(global.OPM_CONFIG.AutoCode.SApi, Package), "enter.go"),
			funcName:    "ApiGroup",
			structNameF: "%sApi",
		},
		{
			path: filepath.Join(global.OPM_CONFIG.AutoCode.Root,
				global.OPM_CONFIG.AutoCode.Server, fmt.Sprintf(global.OPM_CONFIG.AutoCode.SRouter, Package), "enter.go"),
			funcName:    "RouterGroup",
			structNameF: "%sRouter",
		},
		{
			path: filepath.Join(global.OPM_CONFIG.AutoCode.Root,
				global.OPM_CONFIG.AutoCode.Server, fmt.Sprintf(global.OPM_CONFIG.AutoCode.SService, Package), "enter.go"),
	...
	packageInjectionMap = map[string]astInjectionMeta{
		packageServiceName: {
			path: filepath.Join(global.OPM_CONFIG.AutoCode.Root,
				global.OPM_CONFIG.AutoCode.Server, "service", "enter.go"),
			importCodeF:  "github.com/spark8899/ops-manager/server/%s/%s",
			packageNameF: "%s",
		...
		packageRouterName: {
			path: filepath.Join(global.OPM_CONFIG.AutoCode.Root,
				global.OPM_CONFIG.AutoCode.Server, "router", "enter.go"),
			importCodeF:  "github.com/spark8899/ops-manager/server/%s/%s",
			packageNameF: "%s",
		...
		packageAPIName: {
			path: filepath.Join(global.OPM_CONFIG.AutoCode.Root,
				global.OPM_CONFIG.AutoCode.Server, "api/v1", "enter.go"),
			importCodeF:  "github.com/spark8899/ops-manager/server/%s/%s",
			packageNameF: "%s",
			groupName:    "ApiGroup",
...
func (autoCodeService *AutoCodeService) CreateTemp(autoCode system.AutoCodeStruct, ids ...uint) (err error) {
	...
	if autoCode.AutoMoveFile { // 判断是否需要自动转移
...

		{
			// 在gorm.go 注入 自动迁移
			path := filepath.Join(global.OPM_CONFIG.AutoCode.Root,
				global.OPM_CONFIG.AutoCode.Server, global.OPM_CONFIG.AutoCode.SInitialize, "gorm.go")
...

		{
			// router.go 注入 自动迁移
			path := filepath.Join(global.OPM_CONFIG.AutoCode.Root,
				global.OPM_CONFIG.AutoCode.Server, global.OPM_CONFIG.AutoCode.SInitialize, "router.go")
		...
		if err = utils.ZipFiles("./opsmanager.zip", fileList, ".", "."); err != nil {
			return err
		}
	}
...
func (autoCodeService *AutoCodeService) DropTable(BusinessDb, tableName string) error {
	if BusinessDb != "" {
		...
		return global.OPM_DB.Exec("DROP TABLE " + tableName).Error
	}
}
...
func (autoCodeService *AutoCodeService) addAutoMoveFile(data *tplData) {
	...
	if strings.Contains(fileSlice[1], "server") {
		if strings.Contains(fileSlice[n-2], "router") {
			data.autoMoveFilePath = filepath.Join(global.OPM_CONFIG.AutoCode.Root, global.OPM_CONFIG.AutoCode.Server,
				fmt.Sprintf(global.OPM_CONFIG.AutoCode.SRouter, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "api") {
			data.autoMoveFilePath = filepath.Join(global.OPM_CONFIG.AutoCode.Root,
				global.OPM_CONFIG.AutoCode.Server, fmt.Sprintf(global.OPM_CONFIG.AutoCode.SApi, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "service") {
			data.autoMoveFilePath = filepath.Join(global.OPM_CONFIG.AutoCode.Root,
				global.OPM_CONFIG.AutoCode.Server, fmt.Sprintf(global.OPM_CONFIG.AutoCode.SService, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "model") {
			data.autoMoveFilePath = filepath.Join(global.OPM_CONFIG.AutoCode.Root,
				global.OPM_CONFIG.AutoCode.Server, fmt.Sprintf(global.OPM_CONFIG.AutoCode.SModel, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "request") {
			data.autoMoveFilePath = filepath.Join(global.OPM_CONFIG.AutoCode.Root,
				global.OPM_CONFIG.AutoCode.Server, fmt.Sprintf(global.OPM_CONFIG.AutoCode.SRequest, data.autoPackage), base)
		}
	} else if strings.Contains(fileSlice[1], "web") {
		if strings.Contains(fileSlice[n-1], "js") {
			data.autoMoveFilePath = filepath.Join(global.OPM_CONFIG.AutoCode.Root,
				global.OPM_CONFIG.AutoCode.Web, global.OPM_CONFIG.AutoCode.WApi, base)
		} else if strings.Contains(fileSlice[n-2], "form") {
			data.autoMoveFilePath = filepath.Join(global.OPM_CONFIG.AutoCode.Root,
				global.OPM_CONFIG.AutoCode.Web, global.OPM_CONFIG.AutoCode.WForm, filepath.Base(filepath.Dir(filepath.Dir(data.autoCodePath))), strings.TrimSuffix(base, filepath.Ext(base))+"Form.vue")
		} else if strings.Contains(fileSlice[n-2], "table") {
			data.autoMoveFilePath = filepath.Join(global.OPM_CONFIG.AutoCode.Root,
				global.OPM_CONFIG.AutoCode.Web, global.OPM_CONFIG.AutoCode.WTable, filepath.Base(filepath.Dir(filepath.Dir(data.autoCodePath))), base)
		}
	}
}
...
func (autoCodeService *AutoCodeService) AutoCreateApi(a *system.AutoCodeStruct) (ids []uint, err error) {
	...
	err = global.OPM_DB.Transaction(func(tx *gorm.DB) error {
...
func (autoCodeService *AutoCodeService) CreatePackageTemp(packageName string) error {
	...
	for i, s := range pendingTemp {
		pendingTemp[i].path = filepath.Join(global.OPM_CONFIG.AutoCode.Root, global.OPM_CONFIG.AutoCode.Server, filepath.Clean(fmt.Sprintf(s.path, packageName)))
	}
	// 选择模板
...
func (autoCodeService *AutoCodeService) CreatePlug(plug system.AutoPlugReq) error {
	...
	for _, tpl := range tplFileList {
		...
		if strings.Index(pathArr[2], "tpl") < 0 {
			dirPath := filepath.Join(global.OPM_CONFIG.AutoCode.Root, global.OPM_CONFIG.AutoCode.Server, fmt.Sprintf(global.OPM_CONFIG.AutoCode.SPlug, plug.Snake+"/"+pathArr[2]))
			os.MkdirAll(dirPath, 0755)
		}
		file := filepath.Join(global.OPM_CONFIG.AutoCode.Root, global.OPM_CONFIG.AutoCode.Server, fmt.Sprintf(global.OPM_CONFIG.AutoCode.SPlug, plug.Snake+"/"+tpl[len(plugPath):len(tpl)-4]))
...
func (autoCodeService *AutoCodeService) InstallPlugin(file *multipart.FileHeader) (web, server int, err error) {
	const OPMPLUGPINATH = "./opm-plug-temp/"
	defer os.RemoveAll(OPMPLUGPINATH)
	_, err = os.Stat(OPMPLUGPINATH)
	if os.IsNotExist(err) {
		os.Mkdir(OPMPLUGPINATH, os.ModePerm)
	}
	...
	out, err := os.Create(OPMPLUGPINATH + file.Filename)
	if err != nil {
		return -1, -1, err
	}
	defer out.Close()
	...
	paths, err := utils.Unzip(OPMPLUGPINATH+file.Filename, OPMPLUGPINATH)
	...
	if webIndex != -1 {
		err = installation(paths[webIndex], global.OPM_CONFIG.AutoCode.Server, global.OPM_CONFIG.AutoCode.Web)
		if err != nil {
	...
	if serverIndex != -1 {
		err = installation(paths[serverIndex], global.OPM_CONFIG.AutoCode.Server, global.OPM_CONFIG.AutoCode.Server)
	}
	return webIndex, serverIndex, err
}
...
func installation(path string, formPath string, toPath string) error {
	...
	var form = filepath.ToSlash(global.OPM_CONFIG.AutoCode.Root + formPath + "/" + path)
	var to = filepath.ToSlash(global.OPM_CONFIG.AutoCode.Root + toPath + "/plugin/")
