...
import (
	...
	"github.com/spark8899/ops-manager/server/utils/upload"
)
...
func (e *FileUploadAndDownloadService) Upload(file example.ExaFileUploadAndDownload) error {
	return global.OPM_DB.Create(&file).Error
}
...
func (e *FileUploadAndDownloadService) FindFile(id uint) (example.ExaFileUploadAndDownload, error) {
	...
	err := global.OPM_DB.Where("id = ?", id).First(&file).Error
	return file, err
}
...
func (e *FileUploadAndDownloadService) DeleteFile(file example.ExaFileUploadAndDownload) (err error) {
	...
	err = global.OPM_DB.Where("id = ?", file.ID).Unscoped().Delete(&file).Error
	return err
}
...
func (e *FileUploadAndDownloadService) EditFileName(file example.ExaFileUploadAndDownload) (err error) {
	var fileFromDb example.ExaFileUploadAndDownload
	return global.OPM_DB.Where("id = ?", file.ID).First(&fileFromDb).Update("name", file.Name).Error
}
...
func (e *FileUploadAndDownloadService) GetFileRecordInfoList(info request.PageInfo) (list interface{}, total int64, err error) {
	...
	db := global.OPM_DB.Model(&example.ExaFileUploadAndDownload{})
